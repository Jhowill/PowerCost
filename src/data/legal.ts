...
export const LEGAL_UPDATED_AT = '2026-08-13';
...
const privacy: Record<SupportedLocale, LegalDocument> = {
  'pt-BR': {
    ...
    sections: [
      ...
      { title: '2. Dados mantidos no aparelho', paragraphs: ['Cálculos de consumo, histórico, tarifa, idioma, tema, planejamento da casa por cômodo e períodos de fatura, e benefícios temporários são armazenados localmente no aparelho. O PowerCost não possui conta de usuário nem envia esse histórico para um servidor próprio. A disponibilidade da internet é verificada apenas para controlar recursos online e não é armazenada pelo PowerCost.'] },
      ...
      { title: '6. Retenção e segurança', paragraphs: ['Dados locais permanecem até serem apagados nos Ajustes, removidos individualmente ou eliminados com a desinstalação. O backup de dados do app está desativado no Android. O AsyncStorage não deve ser tratado como cofre de informações confidenciais. A retenção de dados publicitários é controlada pelos provedores envolvidos.'] },
  ...
  'en-US': {
    ...
    sections: [
      ...
      { title: '2. Data kept on the device', paragraphs: ['Usage estimates, history, rate, language, theme, household planning by room and bill periods, and temporary benefits are stored locally. PowerCost has no user accounts and does not send this history to its own server. Internet availability is checked only to control online features and is not stored by PowerCost.'] },
      ...
      { title: '6. Retention and security', paragraphs: ['Local data remains until deleted in Settings, removed individually or erased by uninstalling. App data backup is disabled on Android. AsyncStorage should not be treated as a vault for confidential information. Advertising data retention is controlled by the relevant providers.'] },
  ...
  'es-ES': {
    ...
    sections: [
      ...
      { title: '2. Datos guardados en el dispositivo', paragraphs: ['Cálculos, historial, tarifa, idioma, tema, planificación del hogar por habitación y periodos de factura, y beneficios temporales se guardan localmente. PowerCost no tiene cuentas ni envía este historial a un servidor propio. La disponibilidad de internet solo se comprueba para controlar funciones en línea y PowerCost no la almacena.'] },
      ...
      { title: '6. Conservación y seguridad', paragraphs: ['Los datos locales permanecen hasta que se borran en Ajustes, se eliminan individualmente o se desinstala la aplicación. La copia de seguridad de datos está desactivada en Android. AsyncStorage no debe usarse como almacén de información confidencial. Los proveedores controlan la conservación publicitaria.'] },
  ...
  'fr-FR': {
    ...
    sections: [
      ...
      { title: '2. Données conservées sur l’appareil', paragraphs: ['Les calculs, l’historique, le tarif, la langue, le thème, la planification du foyer par pièce et les périodes de facture, ainsi que les avantages temporaires, sont stockés localement. PowerCost ne crée pas de compte et n’envoie pas cet historique à son propre serveur. La disponibilité d’Internet est vérifiée uniquement pour contrôler les fonctions en ligne et n’est pas stockée par PowerCost.'] },
      ...
      { title: '6. Conservation et sécurité', paragraphs: ['Les données locales restent jusqu’à leur suppression dans les Réglages, leur suppression individuelle ou la désinstallation. La sauvegarde des données est désactivée sur Android. AsyncStorage ne doit pas servir de coffre pour des informations confidentielles. Les fournisseurs gèrent la conservation publicitaire.'] },
